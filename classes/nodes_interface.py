...
class NODESInterface:
    ...
    def set_token(self, player_cfg):
        tkn_file_name = "%s%s%s_%s.json" % (
            self.cfg["tokenFilesFolder"],
            os.sep,
            player_cfg["role"],
            player_cfg["id"],
        )
        if os.path.exists(tkn_file_name):
            ...
            if r is False:
                self.logger.warning(
                    "Local token is expired, a new token will be requested to NODES API"
                )
                os.unlink(tkn_file_name)
                self.get_new_token(player_cfg, tkn_file_name)
            else:
                self.logger.info(
                    "Token stored in %s works, no new token will be requested to NODES API"
                    % tkn_file_name
                )
        else:
            self.logger.info(
                "File %s does not exist or work, a new token will be requested to NODES API"
                % tkn_file_name
            )
            self.get_new_token(player_cfg, tkn_file_name)
        ...
        for i in range(0, self.cfg["retries"]):
            ...
            try:
                response = requests.get(
                    endpoint, headers=self.headers, timeout=self.cfg["requestTimeout"]
                )

                if response.status_code == http.HTTPStatus.OK:
                    self.logger.info(
                        "GET endpoint (attempt n. %i): %s, status code: %i"
                        % (i + 1, endpoint, response.status_code)
                    )
                ...
                self.logger.warning(
                    "GET endpoint (attempt n. %i): %s: %s" % (i + 1, endpoint, str(e))
                )
                time.sleep(1)
    ...
    def delete_request(self, endpoint):
        for i in range(0, self.cfg["retries"]):
            ...
            try:
                ...
                if response.status_code == http.HTTPStatus.OK:
                    self.logger.info(
                        "DELETE endpoint (attempt n. %i): %s, status code: %i"
                        % (i + 1, endpoint, response.status_code)
                    )
                    return json.loads(response.text)
                else:
                ...
                self.logger.warning(
                    "DELETE endpoint (attempt n. %i): %s: %s"
                    % (i + 1, endpoint, str(e))
                )
                time.sleep(1)
    ...
    def post_csv_file_request(self, endpoint, tmp_baseline_file):
        for i in range(0, self.cfg["retries"]):
            ...
            try:
                ...
                files = {
                    "file": (
                        tmp_baseline_file.split(os.sep)[-1],
                        open(tmp_baseline_file, "rb"),
                        "text/csv",
                    )
                }
                response = requests.post(
                    ...
                    headers=headers,
                    files=files,
                    timeout=self.cfg["requestTimeout"],
                )

                if response.status_code == http.HTTPStatus.OK:
                    self.logger.info(
                        "POST endpoint (attempt n. %i): %s, status code: %i"
                        % (i + 1, endpoint, response.status_code)
                    )
                    return json.loads(response.text)
                else:
                ...
                self.logger.warning(
                    "POST endpoint (attempt n. %i): %s: %s" % (i + 1, endpoint, str(e))
                )
                time.sleep(1)
    ...
    def patch_request(self, endpoint, data):
        for i in range(0, self.cfg["retries"]):
            ...
            try:
                ...
                if response.status_code == http.HTTPStatus.OK:
                    self.logger.info(
                        "PATCH endpoint (attempt n. %i): %s, status code: %i"
                        % (i + 1, endpoint, response.status_code)
                    )
                    return json.loads(response.text)
                else:
                ...
                self.logger.warning(
                    "PATCH endpoint (attempt n. %i): %s: %s" % (i + 1, endpoint, str(e))
                )
                time.sleep(1)
