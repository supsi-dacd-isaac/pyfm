...
class PostgreSQLInterface:
    ...
    def prepare_tso_query(filter):
        int_sql = (
            "SELECT chg_sf_child.id FROM hive_catalog_rd.site AS chg_sf_child, "
            ...
            "chg_sf_parent.name = '%s' AND chg_sf_parent.active=TRUE AND chg_sf_child.active=TRUE"
            % filter["site_id"]
        )

        sql = (
            "SELECT ctg_site_parent.name AS ss_name, ctg_site_child.city, ctg_site_child.name AS pod, "
            ...
            "chg_sf.active=TRUE AND chg_device_parameter.active=TRUE AND chg_site_device.active=TRUE"
            % int_sql
        )
        if "flexibility_type" in filter.keys():
            sql = "%s AND ctg_device_type.name = '%s'" % (
                sql,
                filter["flexibility_type"],
            )
        sql = (
            "%s ORDER BY ctg_site_parent.name, ctg_site_child.city, ctg_site_child.name"
            % sql
        )
        return sql
    ...
    def prepare_ss_query(filter):
        sql = (
            ...
            "chg_sf.active=TRUE AND chg_device_parameter.active=TRUE"
            % filter["site_id"]
        )
        if "flexibility_type" in filter.keys():
            sql = "%s AND ctg_device_type.name = '%s'" % (
    ...
    def prepare_dso_query(filter):
        sql = (
            "SELECT ctg_device.name AS device_name, ctg_device_type.name AS device_type, chg_device_parameter.properties AS device_properties, ctg_device.id AS device_id, chg_device_parameter.id AS device_parameter_id "
            ...
            "AND ctg_site.name = '%s' AND chg_device_parameter.active = TRUE"
            % filter["site_id"]
        )
        ...
        while row is not None:
            if filter["case"] != "pod":
                ...
                flexi_list.append(
                    {
                        "device_name": row[0],
                        "flexibility_type": row[1],
                        "flexibility_properties": row[2],
                    }
                )
            row = cur.fetchone()
        cur.close()
    ...
    def get_flexibility_metadata(self, site_code, flexi_code):
        sql = (
            "SELECT ctg_device.name AS device_name, chg_device_parameter.properties AS device_properties, "
    ...
    def deactivate_device_parameter(self, device_parameter_id):
        ...
        sql = (
            "UPDATE hive_changelog_rd.device_parameter "
            "SET end_date = '%s', active=FALSE WHERE id = '%s'"
            % (today, device_parameter_id)
        )
        cur = self.conn.cursor()
        cur.execute(sql)
        cur.close()

    def insert_device_parameter(
        self, device_id, device_type_id, organization_id, properties
    ):
        today = datetime.datetime.today().strftime("%Y-%m-%d")
        sql = (
            "INSERT INTO hive_changelog_rd.device_parameter "
    ...
    def save_forecast(
        self, preds, preds_quantiles, day_ahead_electricity_prices, run_dt
    ):
        ...
        day_ahead_electricity_prices.index = (
            day_ahead_electricity_prices.index.tz_convert(pytz.UTC)
        )
        ...
        id_forecast = self.insert_forecast_data(
            site_info["id"], run_dt, "mean", preds[site_info["pod"]]
        )
        self.insert_forecast_data(
            site_info["id"],
            run_dt,
            "day_ahead_price_mean",
            day_ahead_electricity_prices,
        )
        for quantile in preds_quantiles.columns:
            self.insert_forecast_data(
                ...
                run_dt,
                quantile[1],
                preds_quantiles[quantile[0]][quantile[1]],
            )
        return id_forecast

    def save_forecast_controlled(
        self, vcp_id, group_responses_df, preds, scale_factor, run_dt
    ):
        # Transform data index from local time to UTC
        ...
        for device in group_responses_df.loc[:, (slice(None), "tot")].columns:
            ...
            self.insert_forecast_data(
                site_info["id"], run_dt, function_name, group_responses_df[device]
            )
        self.insert_forecast_data(
            site_info["id"],
            run_dt,
            "controlled_all_tot",
            group_responses_df.groupby(level=1, axis=1).sum()["tot"],
        )
        self.insert_forecast_data(
            site_info["id"],
            run_dt,
            "controlled_all_baseline",
            group_responses_df.groupby(level=1, axis=1).sum()["baseline"],
        )
        self.insert_forecast_data(
            site_info["id"],
            run_dt,
            "controlled_all_diff",
            group_responses_df.groupby(level=1, axis=1).sum()["diff"],
        )
        self.insert_forecast_data(
            site_info["id"],
            run_dt,
            "controlled_plus_uncontrolled_all_tot",
            preds[site_info["pod"]].add(
                scale_factor * group_responses_df.groupby(level=1, axis=1).sum()["tot"]
            ),
        )
        self.insert_forecast_data(
            site_info["id"],
            run_dt,
            "controlled_plus_uncontrolled_all_baseline",
            preds[site_info["pod"]].add(
                scale_factor
                * group_responses_df.groupby(level=1, axis=1).sum()["baseline"]
            ),
    ...
    def insert_forecast_data(self, site_id, run_now, function, forecast_data):
        ...
        sql = (
            "INSERT INTO etl.orchestra_forecast (site_id, run_dt, function) VALUES "
            "('%s','%s','%s') RETURNING id"
            % (site_id, run_now.strftime("%Y-%m-%d %H:%M:%S"), function)
        )
    ...
    def get_switch_site_data(self, site_id, flexi_code):
        sql = (
            "SELECT * FROM etl.switch_site "
            "WHERE site_id = '%s' AND load = '%s' AND active=TRUE"
            % (site_id, flexi_code)
        )
        cur = self.conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
        cur.execute(sql)
    ...
    def deactivate_switching_table(self, id_switching_table):
        sql = (
            "UPDATE etl.switching_table SET active=FALSE WHERE id='%s'"
            % id_switching_table
        )
        cur = self.conn.cursor()
        res = cur.execute(sql)
    ...
    def add_switching_table(self, st_evulution_id, st_number, st_name, properties):
        sql = (
            "INSERT INTO etl.switching_table (st_id, number, name, start_dt, properties) VALUES "
            "(%i, %i, '%s','%s', '%s') RETURNING id"
            % (
                st_evulution_id,
                st_number,
                st_name,
    ...
    def add_scheduling_to_switching_table(self, st_id, new_scheduling):
        sql = (
            "INSERT INTO etl.switching_table_smart_manager_scheduling (fk_switching_table_id, trigger_time_utc, "
            ...
            % (
                st_id,
                str(new_scheduling["triggerTimeUtc"]),
                str(new_scheduling["releaseTimeUtc"]),
                new_scheduling["validMonday"],
                new_scheduling["validTuesday"],
                new_scheduling["validWednesday"],
                new_scheduling["validThursday"],
                new_scheduling["validFriday"],
                new_scheduling["validSaturday"],
                new_scheduling["validSunday"],
                datetime.datetime.now().strftime("%Y-%m-%dT%H:%M:%SZ"),
            )
        )
        cur = self.conn.cursor()
    ...
    def add_switch_to_switching_table(self, st_id, smart_manager_id, switch_number):
        sql = (
            "INSERT INTO etl.switching_table_smart_manager_switch (fk_switching_table_id, smart_manager_id, "
            "switch_port, start_dt) VALUES ('%s','%s',%i,'%s')"
            % (
                st_id,
                smart_manager_id,
                switch_number,
    ...
    def get_site_info(self, id_site):
        sql = (
            "SELECT ctg_site.*, ctg_site_type.name AS type "
    ...
    def get_force_off_history(
        self, t_start_local, t_end_local, id_smart_manager, relay
    ):
        # Localize the start/emd time in UTC
        t_start_utc = t_start_local.astimezone(pytz.UTC)
        t_end_utc = t_end_local.astimezone(pytz.UTC)
        ...
        sql = (
            "select stfo.dt, stfo.force_off, st.id "
            ...
            "stsms.smart_manager_id = '%s' and stsms.switch_port=%i order by st.id asc, stfo.dt asc"
            % (
                t_start_utc.strftime("%Y-%m-%d %H:%M:%S"),
                t_end_utc.strftime("%Y-%m-%d %H:%M:%S"),
                id_smart_manager,
        ...
        data = pd.DataFrame(
            list(zip(dt, force_off, st_ids)),
            columns=["dt", "force_off", "switching_table_id"],
        )
    ...
    def get_forecast(self, t_start_local, t_end_local, site_pod, func):
        ...
        sql = (
            "select ofer.run_dt, ofer_ts.dt, ofer_ts.value "
            ...
            "s.pod = '%s' and ofer.function='%s' order by ofer.id asc, ofer.run_dt asc, ofer_ts.dt"
            % (
                t_start_utc.strftime("%Y-%m-%d %H:%M:%S"),
                t_end_utc.strftime("%Y-%m-%d %H:%M:%S"),
                site_pod,
        ...
        data = pd.DataFrame(
            list(zip(run_dts, dts, values)), columns=["run_dt", "dt", "value"]
        )
    ...
    def insert_correspondence_dp_st(self, st_id, dp_id):
        ...
        query = (
            "select id from etl.switching_table_device_parameter where fk_switching_table = '%s' and "
        ...
        if len(data) > 0:
            self.logger.warning(
                "Correspondence [%s:%s] already in table etl.switching_table_device_parameter"
                % (st_id, dp_id)
            )
            return None
        else:
            ...
            sql = (
                "INSERT INTO etl.switching_table_device_parameter(fk_switching_table, "
