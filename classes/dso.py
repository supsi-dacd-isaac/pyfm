...
class DSO(Player):
    ...
    def request_contract(self, dt_slot, fmo):
        if self.cfg["flexibilitySource"] == "random":
            ...
            self.logger.error(
                "Option '%s' not available for creating a contract "
        ...
        body = {
            ...
            "availabilityPrice": self.cfg["contractSection"]["mainSettings"][
                "availabilityPrice"
            ],
            # * * * * *
        ...
        if result is not False:
            ...
            fmo.add_entry_to_contract_request_ledger(
                self.cfg, self.markets[0]["name"], body
            )
        return result
