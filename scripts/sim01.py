...
from scripts.utils import (
    plot_successful_bids_per_bidder,
    plot_unsuccessful_bids_per_bidder,
    plot_combined_bids_per_bidder,
    plot_all_accepted_bids,
    plot_buyer_requests_and_wtp,
    plot_rewards_per_bidder,
    plot_all_bidders_rewards,
    plot_flexibility_from_history,
    plot_flexibility_and_rewards_from_history,
    plot_flexibility_requested_and_rewards_from_history,
)
from scripts.utils_baselines import (
    create_residential_like_pattern,
    create_office_like_pattern,
    create_battery_pattern,
)
...
def test_market_simulation():
    ...
    for step, time_slot in enumerate(time_index):
        ...
        for bidder in bidders:
            ...
            p_start, final_power = bidder.build_offer(
                buyer_id=best_buyer["id"],
                pow_req=best_buyer["requested_power"],
                pow_bid=bidder.baseline.loc[time_slot, "value"] * 0.8,
            )
            bidder.update_current_bidding(
                buyer_id=best_buyer["id"],
                offered_power=final_power,
                offered_price=p_start,
            )
            market_op.receive_bid_from_bidder(
                time_slot=time_slot, bid_info=bidder.current_bidding
            )
        ...
        for bidder in bidders:
            actual_value = bidder.baseline.loc[
                time_slot, "value"
            ] - bidder.current_bidding["power"] * (1 + 0.1 * np.random.randn())
        ...
        for bidder in bidders:
            bidder.add_actual_value(
                time_slot,
                metering_agent.get_energy_data(bidder.id).loc[time_slot, "energy"],
            )
            market_op.store_bidder_actual(
                bidder.id,
                time_slot,
                metering_agent.get_energy_data(bidder.id).loc[time_slot, "energy"],
            )
        ...
        if (step + 1) % clearing_steps == 0:
            ...
            print(
                f"Time slot: {time_slot}: Starting clearing the last {clearing_steps} steps."
            )
            accepted_bids, non_accepted_bids = market_op.pay_as_bid_market_solving(
                clearing_steps
            )
            ...
            for bidder in bidders:
                for bid in accepted_bids.get(time_slot, []):
                    if bid["bidder_id"] == bidder.id:
                        bidder.update_history(
                            bid["buyer_id"], time_slot, bid["price"], bid["power"], True
                        )
            ...
            print(
                f"Time slot: {time_slot}: Ending clearing the last {clearing_steps} steps."
            )
