...
if __name__ == "__main__":
    ...
    now = datetime.utcnow()
    from_str = (now - timedelta(hours=1)).strftime('%Y-%m-%dT%H:%M:%SZ')
    to_str = (now + timedelta(hours=1)).strftime('%Y-%m-%dT%H:%M:%SZ')
    ...
    dso_orders = dso.get_orders(filter_dict={'ownerOrganizationId': dso.organization['id'], 'periodFrom': from_str,
                                             'periodTo': to_str})
    ...
    for fsp_identifier in cfg['fm']['actors']['fsps'].keys():
        ...
        fsp_orders = fsp.get_orders(filter_dict={'ownerOrganizationId': fsp.organization['id'],
                                                 'periodFrom': from_str, 'periodTo': to_str})
