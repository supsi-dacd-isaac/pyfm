...
if __name__ == "__main__":
    ...
    dso_orders = dso.get_orders(
        filter_dict={
            "ownerOrganizationId": dso.organization["id"],
    ...
    for dso_order in dso_orders:
        logger.info(
            "Created: %s, ValidFrom: %s, ValidTo: %s, Side: %s, Type: %s, Quantity: %s, completionType: %s"
            % (
                dso_order["created"],
                dso_order["validFrom"],
                dso_order["validTo"],
                dso_order["side"],
                dso_order["regulationType"],
                dso_order["quantity"],
                dso_order["completionType"],
            )
        )
        ...
        for fsp_order in fsp_orders:
            logger.info(
                ...
                % (
                    ...
                    fsp_order["validTo"],
                    fsp_order["side"],
                    fsp_order["regulationType"],
                    fsp_order["quantity"],
                    fsp_order["completionType"],
                )
            )
