    ...
    for time_slot, results in market_operator.clearing_results_history.items():
        for result in results:
            ...
            for allocation in allocations:
                ...
                bidder_data[bidder_id]["bidded_flexibility"].append(
                    allocation["bidded_flexibility"]
                )
                bidder_data[bidder_id]["real_flexibility"].append(
                    allocation["provided_flexibility"]
                )
...
def plot_flexibility_and_rewards_from_history(market_operator, plot_dir):
    ...
    for bidder_id, data in bidder_data.items():
        ...
        ax1.plot(
            data["time_slots"],
            data["bidded_flexibility"],
            label="Bidded",
            color="tab:blue",
        )
        ax1.plot(
            data["time_slots"],
            data["real_flexibility"],
            label="Provided",
            color="tab:green",
        )
    ...
    for time_slot, results in market_operator.clearing_results_history.items():
        for result in results:
            ...
            for allocation in allocations:
                ...
                bidder_data[bidder_id]["time_slots"].append(time_slot)
                bidder_data[bidder_id]["requested_flexibility"].append(
                    allocation["requested_flexibility"]
                )
                bidder_data[bidder_id]["bidded_flexibility"].append(
                    allocation["bidded_flexibility"]
                )
