...
if __name__ == "__main__":
    ...
    contracts_requests = dso.get_contracts(
        filter_dict={
            "marketId": dso.markets[0]["id"],
            "periodFrom": (slot_time + timedelta(days=7)).strftime(
                "%Y-%m-%dT00:00:00Z"
            ),
    ...
    for contract_request in contracts_requests:
        if (
            "request" in contract_request["name"]
            and contract_request["baseContractId"] is None
        ):
            # Get the id of the contract proposal
            contracts_proposals = dso.get_contracts(
                filter_dict={"baseContractId": contract_request["id"]}
            )
            ...
            if len(contracts_proposals) > 0:
                ...
                logger.warning(
                    "No available contract proposals available for request contract %s",
                    contract_request["id"],
                )
