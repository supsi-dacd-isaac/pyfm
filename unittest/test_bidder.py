...
class TestBidder(unittest.TestCase):
    ...
    def test_select_buyer_with_long_memory(self):
        ...
        for i in range(12):
            self.bidder.update_history(
                "buyer1", f"2024-01-01 00:{i:02d}", 40 + i, 100, i % 2 == 0
            )
            self.bidder.update_history(
                "buyer2", f"2024-01-01 00:{i:02d}", 50 + i, 150, i % 3 == 0
            )
            self.bidder.update_history(
                "buyer3", f"2024-01-01 00:{i:02d}", 60 + i, 200, i % 4 == 0
            )
            self.bidder.update_history(
                "buyer4", f"2024-01-01 00:{i:02d}", 70 + i, 250, i % 5 == 0
            )
