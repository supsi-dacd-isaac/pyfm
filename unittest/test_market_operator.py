...
class TestMarketOperator(unittest.TestCase):
    ...
    def test_receive_buyer_request(self):
        self.market_operator.receive_buyer_request(self.time_slot, {'id': 'buyer1', 'requested_power': 100, 'wtp': 50})
    ...
    def test_receive_bid_from_bidder(self):
        self.market_operator.receive_bid_from_bidder(self.time_slot,
                                                     {'bidder_id': 'bidder1', 'buyer_id': 'buyer1', 'power': 50,
                                                      'price': 40})
    ...
    def test_receive_baseline_from_bidder(self):
        self.market_operator.receive_baseline_from_bidder(self.time_slot, 'bidder1', 100)
        self.assertIn(self.time_slot, self.market_operator.time_slot_baseline)
        self.assertIn('bidder1', self.market_operator.time_slot_baseline[self.time_slot])
        self.assertEqual(self.market_operator.time_slot_baseline[self.time_slot]['bidder1'], 100)
    ...
    def test_get_baseline_for_time_slot(self):
        self.market_operator.receive_baseline_from_bidder(self.time_slot, 'bidder1', 100)
        baseline = self.market_operator.get_baseline_for_time_slot(self.time_slot, 'bidder1')
        self.assertEqual(baseline, 100)

    def test_compute_power_ref(self):
        self.market_operator.receive_buyer_request(self.time_slot, {'id': 'buyer1', 'requested_power': 100, 'wtp': 50})
        self.market_operator.receive_buyer_request(self.time_slot, {'id': 'buyer2', 'requested_power': 200, 'wtp': 60})
    ...
    def test_compute_price_ref(self):
        self.market_operator.time_slot_data = {
            self.time_slot: [{'accepted_price': 40}, {'accepted_price': 50}, {'accepted_price': 60}]
        }
        self.market_operator.compute_price_ref()
        self.assertEqual(self.market_operator.price_ref, 50.0)

    def test_no_demand(self):
        self.market_operator.receive_buyer_request(self.time_slot, {'id': 'buyer1', 'requested_power': 0, 'wtp': 50})
    ...
    def test_partial_fulfillment(self):
        ...
        self.market_operator.store_bidder_baseline('bidder1', pd.DataFrame({'value': [100]}, index=[self.time_slot]))
    ...
    def test_full_fulfillment(self):
        ...
        self.market_operator.receive_bid_from_bidder(self.time_slot,
                                                     {'bidder_id': 'bidder1', 'buyer_id': 'buyer1', 'power': 50,
                                                      'price': 40})
        self.market_operator.receive_bid_from_bidder(self.time_slot,
                                                     {'bidder_id': 'bidder2', 'buyer_id': 'buyer1', 'power': 50,
                                                      'price': 45})

        # Store baseline and actual values for the bidders
        self.market_operator.store_bidder_baseline('bidder1', pd.DataFrame({'value': [100]}, index=[self.time_slot]))
        self.market_operator.store_bidder_baseline('bidder2', pd.DataFrame({'value': [100]}, index=[self.time_slot]))
        self.market_operator.store_bidder_actual('bidder1', self.time_slot, 90)
    ...
    def test_multiple_buyers_bidders(self):
        self.market_operator.receive_buyer_request(self.time_slot, {'id': 'buyer1', 'requested_power': 100, 'wtp': 50})
        self.market_operator.receive_buyer_request(self.time_slot, {'id': 'buyer2', 'requested_power': 150, 'wtp': 60})
        self.market_operator.receive_bid_from_bidder(self.time_slot,
                                                     {'bidder_id': 'bidder1', 'buyer_id': 'buyer1', 'power': 50,
                                                      'price': 40})
        self.market_operator.receive_bid_from_bidder(self.time_slot,
                                                     {'bidder_id': 'bidder2', 'buyer_id': 'buyer1', 'power': 60,
                                                      'price': 45})
        self.market_operator.receive_bid_from_bidder(self.time_slot,
                                                     {'bidder_id': 'bidder3', 'buyer_id': 'buyer2', 'power': 100,
                                                      'price': 55})
        self.market_operator.receive_bid_from_bidder(self.time_slot,
                                                     {'bidder_id': 'bidder4', 'buyer_id': 'buyer2', 'power': 60,
                                                      'price': 50})

        # Store baseline and actual values for the bidders
        self.market_operator.store_bidder_baseline('bidder1', pd.DataFrame({'value': [100]}, index=[self.time_slot]))
        self.market_operator.store_bidder_baseline('bidder2', pd.DataFrame({'value': [100]}, index=[self.time_slot]))
        self.market_operator.store_bidder_baseline('bidder3', pd.DataFrame({'value': [100]}, index=[self.time_slot]))
        self.market_operator.store_bidder_baseline('bidder4', pd.DataFrame({'value': [100]}, index=[self.time_slot]))
        self.market_operator.store_bidder_actual('bidder1', self.time_slot, 90)
        self.market_operator.store_bidder_actual('bidder2', self.time_slot, 90)
    ...
    def test_zero_wtp(self):
        self.market_operator.receive_buyer_request(self.time_slot, {'id': 'buyer1', 'requested_power': 100, 'wtp': 0})
        self.market_operator.receive_bid_from_bidder(self.time_slot,
                                                     {'bidder_id': 'bidder1', 'buyer_id': 'buyer1', 'power': 50,
                                                      'price': 40})
    ...
    def test_store_and_get_bidder_baseline(self):
        ...
        self.assertEqual(self.market_operator.get_bidder_baseline('BIDDER_01', self.time_slot), 1.0)


    def test_store_and_get_bidder_actual(self):
        actual_value = 0.85
        self.market_operator.store_bidder_actual('BIDDER_01', self.time_slot, actual_value)
        self.assertEqual(self.market_operator.get_bidder_actual('BIDDER_01', self.time_slot), actual_value)

    def test_calculate_reward(self):
        price = self.bid['price']
        reward = self.market_operator.calculate_reward(price, self.real_flexibility, self.power_requested)
        expected_reward = (min(self.power_requested, self.real_flexibility) * price -
                           self.market_operator.alpha_rem * max(self.power_requested - self.real_flexibility - (
                           self.market_operator.threshold_rem * self.power_requested), 0) * price +
                           self.market_operator.beta_rem * max(self.real_flexibility - self.power_requested - (
                           self.market_operator.threshold_rem * self.power_requested), 0) * price)
